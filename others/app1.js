...
function captureLilaImage() {
    ...
    const scale = 4;
    saveCanvas.width = PROCESS_WIDTH * scale;
    saveCanvas.height = PROCESS_HEIGHT * scale;
    const saveCtx = saveCanvas.getContext('2d');
    
    // Turn off smoothing for pixel art look
    saveCtx.imageSmoothingEnabled = false;
    
    // Draw current low-res output to high-res canvas
    saveCtx.drawImage(outputCanvas, 0, 0, saveCanvas.width, saveCanvas.height);
    
    // Download
    const dataURL = saveCanvas.toDataURL('image/png');
    const link = document.getElementById('downloadLink');
    const date = new Date();
    const filename = `LILA_EVIDENCE_${date.getHours()}${date.getMinutes()}${date.getSeconds()}.png`;
    ...
    if (captureBtn) {
        ...
        captureBtn.style.background = "var(--lila-red)";
